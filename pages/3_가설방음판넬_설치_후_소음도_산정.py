...
# --- [핵심] 서버 환경 무관하게 한글 폰트 자동 다운로드 및 적용 ---
...
with st.sidebar:
    ...
    dist_source_barrier = st.number_input("음원과 가설방음판넬까지의 거리(m)", value=10.0, step=0.1, format="%.1f")
    dist_receiver_barrier = st.number_input("이격거리(m)", value=30.0, step=0.1, format="%.1f")
...
st.markdown(
    ...
    " padding: 10px;'><b>제작자 :</b> (주)내경엔지니어링 박은정 과장(eunjeong0901@naver.com)</div>",
